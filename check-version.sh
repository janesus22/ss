...
VERSION_UP=$(curl -s -f -H "Authorization: Bearer ${TOKEN}" https://${API_DOMAIN}/v2/${DOCKER_HUB_UP}/tags/list | jq -r '.tags[]' | grep -E '^'"${MAJOR_VERSION}"'\.[0-9.]+' | sort --version-sort | tail -n 1 | cut -d'-' -f1)
[ "$?" -ne 0 ] && exit 1
